<?php
function object_cmp ($lhs_obj, $rhs_obj) {
	return strcmp($lhs_obj->meta_value, $rhs_obj->meta_value);
}
class ECF_Field {
	var $type;
	...
	function save() {
		...
		if ($this->is_multiply) {
			...
			if (get_class($this) == 'ECF_FieldMap') {
				...
				// $check_query = "SELECT * FROM wp_doctors_locations WHERE wp_doctors_locations.user_id='" . $doctor_id . "'";
				// $query_results = $wpdb->get_results($check_query);
	...
	function render_field() {
		...
		if ($this->is_multiply) {
			...
			//Sorting the objects by this alphabetic values
			usort($temp_values, 'object_cmp');


			//Restoring the old numeric keys
			for ($i = 0; $i < $temp_values_length; $i++) {
				foreach ($this->options as $key => $value) {
					if ($temp_values[$i]->meta_value == $value) {
						$temp_values[$i]->meta_value = $key;
					}
				}
			}
			*/
			...
			foreach ($this->values as $val) {
				...
				if (isset($this->options)) {
                    ...
                    if (array_key_exists($field->value, $this->options)) {
                        $return .= $field->render();
                    } else {
                        // If the field value doesn't exist in the list of options then the item was a dependency
                        // that has been removed. In this case, we want to make sure that the meta information is
                        // removed so that it doesn't attempt to load it in the future.
                        delete_post_meta($field->post_id, $this->name, $field->value);
                    }
                }
			}
		}
...
class ECF_FieldMultiplySelect extends ECF_Field {
	private $ecf_count_value;
	...
	function render() {
		...
		//$final_html .= $this->build_tag('input', $input_atts);
		//$this->value .= ';' . $this->ecf_count_value;
		return $this->render_row($final_html);
	}
}
...
class ECF_FieldForeignKey extends ECF_FieldSelect {
	...
	function lazy_loader() {
	    ...
	    $entries = get_posts('showposts=-1&posts_per_page=-1&post_type=' . $this->post_type);
		$entries_map = array();
		if ($this->is_optional) {
